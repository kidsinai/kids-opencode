...
|---|---|---|---|
| 1 | `curl -fsSL https://airbotix.ai/install/kids \| sh` works on clean macOS + Linux | 🟡 installer + wrapper + register + --update + drift-check all engineered; installs both plugin and kids-client | npm publish + airbotix.ai endpoint deployment (Airbotix-AI/airbotix PR #36) |
| 2 | `@kidsinai/kids-opencode-plugin` + `@kidsinai/kids-client` published to npm | 🔴 | npm `@kidsinai` scope auth (Lightman) |
...
| 7 | Public privacy policy + ToS + parental consent forms live on airbotix.ai | 🟢 → 🟡 4 docs drafted **AND rendered + footer-linked** on airbotix.ai (verified 2026-05-16); only lawyer review remains | lawyer review |
| 8 | Red-team test set ≥48/50 pass rate | 🟡 50 prompts written (`docs/red-team.md`); not yet executed | provider key |
| 9 | Plugin emits audit lines with v1 schema envelope that platform-backend can ingest | 🟢 plugin V0.5 stamps `schema_version: "1"` + `identity: {family_id, kid_profile_id, device_id, workshop_class_id}` on every audit line per audit-event-schema PRD §3.2; remote POST endpoint awaiting platform-backend | platform-backend `/api/audit` endpoint |

**Engineering net**: this side of the wall is done. Items 2-3, 5-6, 8 all depend on actions outside this repo's scope. Item 9 is engineering-complete on the plugin side; downstream platform-backend ingest is the remaining gap.
...
| Version | Date | Note |
|---|---|---|
| 0.6 | 2026-05-16 | **Phase 2.5 MVP + full TUI complete (airbotix session).** New `packages/kids-client/` (`@kidsinai/kids-client`) — Ink (Node+React) own-client TUI per kids-opencode-client-prd.md v0.3 C-route + D-CL7. 32 files / ~2300 LOC; pure-TS `core/` (env / serve-manager / connection / session / events / store / audit-pipeline / course-pack / check-runner / last-session) + `render/ink/` (App router, theme, 7 screens, 6 components). PRD §3 fully covered: Startup [Enter/c/r/h] / Mission progress + Stars / Permission y/n/e modal w/ Stars cost preview / DangerousTopic Kids Helpline overlay / 6-variant ErrorScreen / HelpScreen / CoursePackPicker / MissionComplete ASCII fireworks / LoadingScreen. Esc-to-abort streaming. `/check`+`我做完了` detection → in-process `runMissionChecks()`. Toast system (reconnect / compaction / mission-next-started). PRD §7 onboarding: `register` subcommand (device-code stub, awaits airbotix-app portal callback), `--update` self-update, `--shutdown` safety hatch. Day-0 Q3 spike resolved: Path B chosen (client owns serve + tails stderr; PRD §5.3 session-resume V1-deferred). 55 client tests + 48 plugin tests + 44 tui-plugin tests = **147 tests, all green**. Plugin V0.5 adds `schema_version: "1"` + `identity:{family_id,kid_profile_id,device_id,workshop_class_id}` to every audit line per audit-event-schema PRD §3.2. Upstream sync gauntlet (`packages/kids-plugin/test/gauntlet/`) + workflow (`.github/workflows/upstream-bump-gauntlet.yml`) wired. `install.sh` now installs both plugin and kids-client. |
